...
void LaunchVmi_group_reduce_i16_addi_full_chunk_store_kernel(int16_t *src,
                                                             int16_t *dst,
                                                             void *stream);

int main() {
  constexpr size_t kInputElems = 512;
  constexpr size_t kOutputElems = 16;
  size_t srcBytes = kInputElems * sizeof(int16_t);
  size_t dstBytes = kOutputElems * sizeof(int16_t);
  ...
  ACL_CHECK(aclrtMemcpy(dstDevice, dstBytes, dstHost, dstBytes,
                        ACL_MEMCPY_HOST_TO_DEVICE));
  LaunchVmi_group_reduce_i16_addi_full_chunk_store_kernel(srcDevice, dstDevice,
                                                          stream);
